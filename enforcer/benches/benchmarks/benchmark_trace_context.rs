...
fn benchmark_get_trace_context(c: &mut Criterion) {
    ...
    group.bench_function("get_trace_context_with_active_span", |b| b.iter(get_trace_context));

    // Also benchmark when there is no active span context
    drop(_guard);
    group.bench_function("get_trace_context_without_active_span", |b| b.iter(get_trace_context));
}
