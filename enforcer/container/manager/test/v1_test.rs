...
use container_manager::sanitize_path_component;
use container_manager_request::{MountReadOnlyFile, MountWritableFile, ResetIsolateRequest};
...
async fn test_otel_metrics_enrichment_isolate_name() {
    ...
    let resource_attrs = &received.resource_metrics[0]
        .resource
        .as_ref()
        .expect("Resource should be present")
        .attributes;
    let find_resource_attr = |key: &str| {
        resource_attrs
            .iter()
