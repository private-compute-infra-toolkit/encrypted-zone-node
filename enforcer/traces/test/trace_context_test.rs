...
use tonic::Request;
use traces::context::{extract_parent_context, get_trace_context, TonicHeaderExtractor};
...
#[test]
fn test_extract_parent_context_none() {
    // Set a current span to verify it's returned
    let provider = sdktrace::SdkTracerProvider::default();
    let tracer = provider.tracer("test");
    ...
    let extracted_cx = extract_parent_context(None);
    assert_eq!(
...
#[test]
fn test_extract_parent_context_empty_headers() {
    let provider = sdktrace::SdkTracerProvider::default();
    let tracer = provider.tracer("test");
    let span = tracer.start("parent-span");
    let cx = Context::current_with_span(span);
    ...
    let extracted_cx = extract_parent_context(Some(&headers));
    assert_eq!(
        extracted_cx.span().span_context().trace_id(),
        Context::current().span().span_context().trace_id()
    );
...
#[test]
fn test_extract_parent_context_valid_headers() {
    let provider = sdktrace::SdkTracerProvider::default();
    let tracer = provider.tracer("test");
    let span = tracer.start("parent-span");
    let cx = Context::current_with_span(span);
    ...
    assert_eq!(extracted_span_context.trace_id().to_string(), "4bf92f3577b34da6a3ce929d0e0e4736");
}

#[test]
fn test_extract_parent_context_invalid_headers() {
    let provider = sdktrace::SdkTracerProvider::default();
    let tracer = provider.tracer("test");
    let span = tracer.start("parent-span");
    let cx = Context::current_with_span(span);
    ...
    let mut headers = std::collections::HashMap::new();
    headers.insert("traceparent".to_string(), "invalid-traceparent-format".to_string());
